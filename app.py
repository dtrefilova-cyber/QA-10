...
Правила оцінювання:
1. Дружелюбне питання/Мета дзвінка: якщо є дружнє питання або озвучена мета дзвінка → 2.5; якщо відсутнє → 0.
2. Спроба продовжити розмову: якщо менеджер намагається продовжити діалог → 5; якщо погоджується перервати → 0.
3. Домовленність про наступний контакт: дата+час → 10; день/дата+частина дня → 7.5; тільки дата/день → 5; ≥2 спроби домовитись без відповіді → 10.
4. Пропозиція бонусу: не згадано → 0; згадано без умов → 5; згадано з умовами → 10.
5. Передзвон клієнту: «так, був протягом години» → 10; «так, був протягом 3 годин» → 5; «ні, не було» + домовленість → 0; «ні, не було» + домовленості немає → 10.
6. Професіоналізм: правильний бонус → 10; неправильний бонус → 5; заборонені слова → 0.
7. Робота із запереченнями: заперечень не було → 10; було, але проігноровано → 0.
8. Зливання клієнта: шукає причину завершити → 0; пасивний → 10; активно залучений → 15.
