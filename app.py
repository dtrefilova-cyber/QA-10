...
def clean_and_structure(dialogue):
    ...
    prompt = f"""
Ти отримуєш транскрипт телефонної розмови після speech-to-text.

ВАЖЛИВО:
Це не звичайний текст — це розпізнавання аудіо, тому можуть бути:
- перекручені слова
- злиті слова
- неправильні форми слів
- фрази, які звучать нелогічно

ТВОЯ ЗАДАЧА:
1. Виправляти помилки розпізнавання слів і фраз, щоб вони звучали природно
   приклади:
   - "спільні" → "спіни"
   - "безвейджеру" → "без вейджеру"
   - "я до вас знаходжу по другому" → "я до вас на хвилинку-другу телефоную"
   - "я на роботу" → "я на роботі"

2. МОЖНА:
- виправляти слова
- розділяти слова
- трохи коригувати фразу, якщо вона очевидно зламана

3. НЕ МОЖНА:
- вигадувати нову інформацію
- змінювати сенс сказаного
- скорочувати текст

4. Зберегти стиль мовлення (укр/рос/суржик)

5. Замінити:
- ch_0 → Менеджер
- ch_1 → Клієнт

Поверни тільки готовий діалог.
"""
